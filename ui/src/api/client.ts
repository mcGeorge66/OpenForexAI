...
export const api = {
  ...
  // ── Event Log ───────────────────────────────────────────────────────────────
  getEventSchemas: () => get<Record<string, EventSchemaEntry>>('/config/event_schemas'),
  saveEventSchemas: (content: Record<string, unknown>) =>
    put<{ status: string; schema_count?: number }>('/config/event_schemas', content),
  reloadEventSchemas: () => post<{ status: string; schema_count: number }>('/config/reload/event_schemas', {}),
  getEvents: (params?: EventLogParams) => {
    const q = new URLSearchParams()
    if (params?.event_type) q.set('event_type', params.event_type)
    if (params?.source_agent) q.set('source_agent', params.source_agent)
    if (params?.correlation) q.set('correlation', params.correlation)
    if (params?.from_time) q.set('from_time', params.from_time)
    if (params?.to_time) q.set('to_time', params.to_time)
    if (params?.trace_roots_only) q.set('trace_roots_only', 'true')
    if (params?.chain_min !== undefined) q.set('chain_min', String(params.chain_min))
    if (params?.chain_max !== undefined) q.set('chain_max', String(params.chain_max))
    if (params?.limit !== undefined) q.set('limit', String(params.limit))
    if (params?.offset !== undefined) q.set('offset', String(params.offset))
    const suffix = q.toString() ? `?${q.toString()}` : ''
    return get<EventLogEntry[]>(`/events${suffix}`)
  },
  getEvent: (id: string) => get<EventLogEntry>(`/events/${encodeURIComponent(id)}`),
  getEventTrace: (id: string) => get<EventLogEntry[]>(`/events/${encodeURIComponent(id)}/trace`),
}
