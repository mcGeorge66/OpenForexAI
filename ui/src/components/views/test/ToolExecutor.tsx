...
export function ToolExecutor() {
  ...
  useEffect(() => {
    api.getAgents()
       .then(resp => {
        setAgentOptions(resp.map(a => a.agent_id))
      })
    ...
    api.getConfigView()
      .then(cfg => {
        ...
        const pairs = Array.from(new Set(
          Object.values(nextAgentConfigs)
            .map(c => c.pair)
            .filter(p => p && p !== 'ALL___' && p !== 'ALL...'),
        )).sort()
      ...
      <div className="flex-1 min-h-0 p-4 flex flex-col gap-4 overflow-hidden">
        ...
        <div>
          ...
          >
            ...
            {tools.slice().sort((a, b) => a.name.localeCompare(b.name)).map(t => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0 items-stretch">
              ...
              <div className="flex flex-col min-h-0 h-full">
                ...
                {resultJson === null ? (
                  ...
                  const displayText = replaceNewlines
                    ? resultJson.replace(/\\n/g, '\n')
                    : resultJson
                  const boxClass = `text-xs p-3 rounded border overflow-auto flex-1 ${
                    isError ? 'bg-red-950/30 border-red-800' : 'bg-gray-900 border-gray-700'
                  }`
                  if (renderMarkdown) {
                    return (
                      <article className={[
                        boxClass,
                        'font-sans max-w-none text-sm text-gray-200 leading-7',
                        '[&_h1]:text-2xl [&_h1]:font-semibold [&_h1]:text-white [&_h1]:mt-5 [&_h1]:mb-3 [&_h1]:border-b [&_h1]:border-gray-700 [&_h1]:pb-2',
                        ...
                        '[&_h4]:text-sm [&_h4]:font-semibold [&_h4]:text-gray-100 [&_h4]:mt-3 [&_h4]:mb-1',
                        '[&_p]:my-2 [&_p]:text-gray-300',
