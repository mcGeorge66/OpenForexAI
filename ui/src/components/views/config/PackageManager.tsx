...
export function PackageManager() {
  ...
  const [includeDecisionPrompts, setIncludeDecisionPrompts] = useState(true)
  const [includeBridgeTools, setIncludeBridgeTools] = useState(true)
  const [includeEventRouting, setIncludeEventRouting] = useState(true)
  const [includeSystemConfig, setIncludeSystemConfig] = useState(false)
  ...
  const [importDecisionPrompts, setImportDecisionPrompts] = useState(true)
  const [importBridgeTools, setImportBridgeTools] = useState(true)
  const [importEventRouting, setImportEventRouting] = useState(true)
  const [importSystemConfig, setImportSystemConfig] = useState(false)
  ...
  const includeAnyExport = includeAgents || includeSnapshots || includeDecisionPrompts || includeBridgeTools || includeEventRouting || includeSystemConfig
  ...
  const doExport = async () => {
    if (!includeAnyExport) {
      setError('Select at least one configuration area to export.')
      return
    }
    if (includeAgents && selected.size === 0) {
      setError('Select at least one agent or disable agent export.')
      return
    }
    setLoading(true)
    setError(null)
    ...
    try {
      const resp = await api.exportAgentPackage({
        ...
        agent_ids: includeAgents ? selectedList : [],
        include_snapshot_profiles: includeSnapshots,
        include_decision_prompt_profiles: includeDecisionPrompts,
        include_bridge_tools: includeBridgeTools,
        include_event_routing: includeEventRouting,
        include_system_config: includeSystemConfig,
        strict_dependencies: strictDependencies,
      })
      setPackageText(resp.text)
      const ts = new Date().toISOString().replace(/[:.]/g, '-')
      downloadText(`ofai-config-package-${ts}.json5`, resp.text)
      setMessage('Configuration package exported.')
    } catch (err) {
      setError(String(err))
    } finally {
      setLoading(false)
  ...
  const doImport = async () => {
    ...
    try {
      const resp: PackageImportResponse = await api.importAgentPackage({
        ...
        mapping,
        replace_existing_agents: replaceExisting,
        import_agents: importAgents,
        import_snapshot_profiles: importSnapshots,
        import_decision_prompt_profiles: importDecisionPrompts,
        import_bridge_tools: importBridgeTools,
        import_event_routing: importEventRouting,
        import_system_config: importSystemConfig,
      })
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 p-4 bg-gray-950 overflow-auto space-y-4">
        ...
        <section className="border border-gray-700 rounded p-3 bg-gray-900/40 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 text-xs text-gray-300">
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeAgents} onChange={e => setIncludeAgents(e.target.checked)} />Agents</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeSnapshots} onChange={e => setIncludeSnapshots(e.target.checked)} />Snapshot Profiles</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeDecisionPrompts} onChange={e => setIncludeDecisionPrompts(e.target.checked)} />Decision Prompt Profiles</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeBridgeTools} onChange={e => setIncludeBridgeTools(e.target.checked)} />Bridge Tools</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeEventRouting} onChange={e => setIncludeEventRouting(e.target.checked)} />Event Routing</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeSystemConfig} onChange={e => setIncludeSystemConfig(e.target.checked)} />System Config</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={strictDependencies} onChange={e => setStrictDependencies(e.target.checked)} />Strict agent dependencies</label>
          </div>
        ...
        <section className="border border-gray-700 rounded p-3 bg-gray-900/40 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 text-xs text-gray-300">
            ...
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={importAgents} onChange={e => setImportAgents(e.target.checked)} />Import agents</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={importSnapshots} onChange={e => setImportSnapshots(e.target.checked)} />Import snapshot profiles</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={importDecisionPrompts} onChange={e => setImportDecisionPrompts(e.target.checked)} />Import decision prompt profiles</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={importBridgeTools} onChange={e => setImportBridgeTools(e.target.checked)} />Import bridge tools</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={importEventRouting} onChange={e => setImportEventRouting(e.target.checked)} />Import event routing</label>
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={importSystemConfig} onChange={e => setImportSystemConfig(e.target.checked)} />Import system config</label>
          </div>

          <div className="flex items-center gap-2 justify-end">
            <button onClick={doValidate} disabled={loading || !packageText.trim()} className="text-xs px-3 py-1.5 rounded bg-blue-700 hover:bg-blue-600 text-white disabled:opacity-50">Validate</button>
