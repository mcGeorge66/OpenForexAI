...
import { RefreshCw, Save, Trash2, ChevronUp, ChevronDown, ChevronsUpDown, MessageSquare, Lock } from 'lucide-react'
...
type RoutingRule = {
  ...
  priority: number
  disable?: boolean
  /** Who maintains this rule. Absent or "rule" = hand-written here; anything
   *  else names the service that derives it from its own config. */
  owner?: string
}
...
const DESIGNER_OWNER = 'rule'

/** Derived rules are generated by a service and rebuilt on every sync —
 *  editing them here would be silently undone, so they are read-only. */
const ruleOwner = (r: RoutingRule): string => r.owner || DESIGNER_OWNER
const isDerived = (r: RoutingRule): boolean => ruleOwner(r) !== DESIGNER_OWNER
...
export function EventRoutingEditor() {
  ...
  const persist = async (nextRules: RoutingRule[], nextSelected: number | null, okMsg: string) => {
    ...
    const keepId = nextSelected !== null ? nextRules[nextSelected]?.id : null
    setSaving(true); setError(null); setMessage(null)
    try {
      await api.saveConfigFile('event_routing', payload)
      // Re-read instead of trusting what we sent: the backend keeps the rules
      // owned by other services and stores the file in priority order, so the
      // saved result is not necessarily the array we handed over.
      const next = await api.getConfigFile('event_routing') as RoutingConfig
      const savedRules = Array.isArray(next.rules) ? next.rules : []
      const idx = keepId ? savedRules.findIndex(r => r.id === keepId) : -1
      setCfg(next); setRules(savedRules); setSelectedIndex(idx >= 0 ? idx : null)
      setForm(idx >= 0 ? savedRules[idx] : EMPTY_RULE)
      setMessage(okMsg)
    } catch (err) { setError(String(err)) }
  ...
  // A derived rule is rebuilt from its owner's config on every sync, so any
  // edit here would be silently overwritten — show it, don't let it be changed.
  const formIsDerived = isDerived(form)
              ...
              <table className="w-full text-xs">
                <thead className="bg-gray-900 text-gray-300 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="text-left px-2 py-2 w-20 bg-gray-900">Quelle</th>
                ...
                <tbody>
                    ...
                    <tr key={`${r.id}-${idx}`} onClick={() => selectRule(idx)}
                      ...
                      <td className="px-2 py-1.5">
                        {isDerived(r) ? (
                          ...
                          <span className="text-[10px] text-gray-600">statisch</span>
                        )}
                      </td>
                  ...
                  {visibleRows.length === 0 && (
                    <tr><td colSpan={9} className="px-3 py-3 text-gray-600 text-center">No rules match the current filter.</td></tr>
                  )}
                </tbody>
              </table>
              </div>
              ...
              <section className="border border-gray-700 rounded p-3 bg-gray-900/40 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => void handleUpdate()} disabled={saving || formIsDerived}
                    ...
                    <button onClick={() => void handleSaveAsNew()} disabled={saving || formIsDerived}
                      className="text-xs px-3 py-1.5 rounded bg-blue-700 hover:bg-blue-600 text-white disabled:opacity-50">
                      Save As New
                    </button>
                    <button onClick={() => void handleDelete()} disabled={saving || selectedIndex === null || formIsDerived}
                ...
                {formIsDerived && (
                  <div className="flex items-start gap-2 rounded border border-violet-600/40 bg-violet-950/40 px-3 py-2 text-xs text-violet-200">
                    <Lock className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />
                    <span>
                      Dynamische Regel — erzeugt vom Dienst <span className="font-mono text-violet-100">{ruleOwner(form)}</span> aus
                      dessen eigener Konfiguration ({ruleOwner(form) === 'telegram'
                        ? <span className="font-mono">notifications.rules</span>
                        : 'Dienst-Konfiguration'}). Sie wird bei jeder Synchronisierung neu aufgebaut,
                      Änderungen hier wären nach dem nächsten Start wieder weg. Zum Ändern oder Löschen
                      die Quelle bearbeiten, nicht diese Regel.
                    </span>
                  </div>
                )}

                <fieldset disabled={formIsDerived} className="space-y-3 disabled:opacity-60">
