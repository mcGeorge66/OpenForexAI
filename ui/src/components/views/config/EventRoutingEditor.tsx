...
export function EventRoutingEditor() {
  ...
  const fieldHelp = useMemo(() => {
    const map: Record<string, string> = {
      ...
      comment:     'Personal notes for this rule. Has no effect on routing — purely for documentation.',
  ...
  return (
              ...
              <section className="border border-gray-700 rounded p-3 bg-gray-900/40 space-y-3">
                ...
                <label className="block text-xs text-gray-300">
                  ...
                  <textarea value={form.comment ?? ''} onChange={e => setField('comment', e.target.value)} onFocus={() => setFocusedField('comment')}
                    rows={2} placeholder="Personal notes for this rule…"
    ...
    {aiAssistantOpen && (
      <AiAssistantModal
        title="AI Assistant — Event Routing"
        contextFile="event_routing_assistant.md"
        contextData={JSON.stringify(form, null, 2)}
        contextDataLabel={form.id || 'new rule'}
