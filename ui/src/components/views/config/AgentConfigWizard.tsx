...
export function AgentConfigWizard() {
  ...
  return (
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-gray-700 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-white">{root ? joinPath(root, 'config', 'config.json5') : 'config/config.json5'} (agents)</span>
          <button
            onClick={() => void load()}
    ...
    {aiAssistantOpen && (
      <AiAssistantModal
        title="AI Assistant — Agent Config"
        contextFile="agent_config_assistant.md"
        contextData={JSON.stringify(form, null, 2)}
        contextDataLabel={form.agent_id || 'new agent'}
