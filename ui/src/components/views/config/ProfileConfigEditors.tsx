...
type SnapshotToolBlockForm = {
  _reactKey: string   // stable identity — never changes, never serialised
  id: string
  tool_name: string
  output_key: string
  enabled: boolean
...
const EMPTY_SNAPSHOT: SnapshotProfileForm = {
  ...
  tool_blocks: [
    { _reactKey: crypto.randomUUID(), id: 'm5_recent',   tool_name: 'get_candles',         output_key: 'm5_recent',   enabled: true, arguments: { timeframe: 'M5',  count: '20'  }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'h1_recent',   tool_name: 'get_candles',         output_key: 'h1_recent',   enabled: true, arguments: { timeframe: 'H1',  count: '120' }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'ema_fast',    tool_name: 'calculate_indicator', output_key: 'ema_fast',    enabled: true, arguments: { indicator: 'EMA', period: '20', timeframe: 'H1', history: '3' }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'ema_slow',    tool_name: 'calculate_indicator', output_key: 'ema_slow',    enabled: true, arguments: { indicator: 'EMA', period: '50', timeframe: 'H1', history: '3' }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'rsi_primary', tool_name: 'calculate_indicator', output_key: 'rsi_primary', enabled: true, arguments: { indicator: 'RSI', period: '7',  timeframe: 'H1', history: '3' }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'atr_primary', tool_name: 'calculate_indicator', output_key: 'atr_primary', enabled: true, arguments: { indicator: 'ATR', period: '7',  timeframe: 'H1', history: '3' }, transform_script: '' },
  ],
  calculation_blocks: [],
...
function findAutoScript(toolName: string, args: Record<string, string>, snippets: SnippetLibraryEntry[]): string {
  const indicator = args.indicator ? `_${args.indicator.toUpperCase()}` : ''
  if (indicator) {
    const specific = snippets.find(s => s.id === `auto_${toolName}${indicator}`)
    if (specific) return specific.code ?? ''
  }
  const generic = snippets.find(s => s.id === `auto_${toolName}`)
  return generic?.code ?? ''
}
...
function buildLegacySnapshotToolBlocks(raw: Record<string, unknown>): SnapshotToolBlockForm[] {
  ...
  return [
    { _reactKey: crypto.randomUUID(), id: 'm5_recent', tool_name: 'get_candles', output_key: 'm5_recent', enabled: true, arguments: { timeframe: 'M5', count: String(m5Count) }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'h1_recent', tool_name: 'get_candles', output_key: 'h1_recent', enabled: true, arguments: { timeframe: 'H1', count: String(h1Count) }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'ema_fast', tool_name: 'calculate_indicator', output_key: 'ema_fast', enabled: true, arguments: { indicator: 'EMA', period: '20', timeframe: 'H1', history: '3' }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'ema_slow', tool_name: 'calculate_indicator', output_key: 'ema_slow', enabled: true, arguments: { indicator: 'EMA', period: '50', timeframe: 'H1', history: '3' }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'rsi_primary', tool_name: 'calculate_indicator', output_key: 'rsi_primary', enabled: true, arguments: { indicator: 'RSI', period: '7', timeframe: 'H1', history: '3' }, transform_script: '' },
    { _reactKey: crypto.randomUUID(), id: 'atr_primary', tool_name: 'calculate_indicator', output_key: 'atr_primary', enabled: true, arguments: { indicator: 'ATR', period: '7', timeframe: 'H1', history: '3' }, transform_script: '' },
...
function normalizeSnapshotToolBlock(raw: unknown, index: number): SnapshotToolBlockForm | null {
  ...
  return {
    ...
    transform_script: toText(row.transform_script),
  }
}
...
export function SnapshotConfigEditor() {
  ...
  const [chartshotConfigs, setChartshotConfigs] = useState<string[]>(['default'])
  ...
  const [snippetsForAutoScript, setSnippetsForAutoScript] = useState<SnippetLibraryEntry[]>([])
  ...
  const [previewRightTab, setPreviewRightTab] = useState<'decision' | 'trigger'>('decision')
  const [toolRightTab, setToolRightTab] = useState<'transformed' | 'arguments'>('transformed')
  ...
  const load = async () => {
    ...
    try {
      ...
      const allSnippets = snippetLib.snippets ?? []
      const parseTags = (s: SnippetLibraryEntry) => (s.tags ?? '').split(',').map(t => t.trim())
      const calcSnippets = allSnippets.filter(s => parseTags(s).includes('SnapshotCalc'))
      const autoSnippets = allSnippets.filter(s => parseTags(s).includes('AutoScript'))
      setSnippetsForCalc(calcSnippets)
      setSnippetsForAutoScript(autoSnippets)
      ...
      const csConfigs = csSection && typeof csSection === 'object'
        ...
        : ['default']
      setChartshotConfigs(csConfigs.length > 0 ? csConfigs : ['default'])

      // Unique pairs from agent configs
      const agentCfgs = (next as Record<string, unknown>).agents
      const pairs = agentCfgs && typeof agentCfgs === 'object'
        ? [...new Set(
            Object.values(agentCfgs as Record<string, Record<string, unknown>>)
              .map(a => typeof a.pair === 'string' ? a.pair.toUpperCase() : '')
              .filter(Boolean)
          )].sort()
        : []
      ...
      return {
        ...
        tool_blocks: [
          ...prev.tool_blocks,
          {
            ...
            transform_script: findAutoScript(toolCandidate, defaultArgumentsForTool(toolCandidate), snippetsForAutoScript),
          },
        ],
      }
    })
  ...
  return (
    <div className="flex flex-col h-full">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title="Open AI Assistant for Snapshot Config"
                    >
                                  ...
                                  <label className="text-xs text-gray-300">
                                    ...
                                    <select
                                      ...
                                      onChange={e => updateToolBlock(index, {
                                        ...
                                        transform_script: findAutoScript(e.target.value, defaultArgumentsForTool(e.target.value), snippetsForAutoScript),
                                      })}
                                      className="mt-1 w-full bg-gray-800 border border-gray-600 rounded px-2 py-1 text-sm text-gray-200"
                                    >
                                        ...
                                        <label key={`${block._reactKey}-${argName}`} className="col-span-4 text-xs text-gray-300 flex h-full flex-col justify-end">
                                          ...
                                          {block.tool_name === 'chartshot' && argName === 'pair' ? (
                                            <select
                                              ...
                                              className="mt-1 w-full bg-gray-800 border border-gray-600 rounded px-2 py-1 text-sm text-gray-200"
                                            >
                                              <option value="">-- agent pair (default) --</option>
                                              {availablePairs.map(p => <option key={p} value={p}>{p}</option>)}
                                            </select>
                                          ) : block.tool_name === 'chartshot' && argName === 'config' ? (
                                            <select
                                              value={value}
                                              onChange={e => setToolBlockArgument(index, argName, e.target.value)}
                                              className="mt-1 w-full bg-gray-800 border border-gray-600 rounded px-2 py-1 text-sm text-gray-200"
                                            >
                                              ...
                                              {chartshotConfigs.map(c => <option key={c} value={c}>{c}</option>)}
                                            </select>
                                          ) : prop.enum ? (
                                            <select
                                              value={value}
      ...
      {previewOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6">
          <div className="w-full max-w-6xl max-h-[88vh] bg-gray-950 border border-gray-700 rounded-xl overflow-hidden flex flex-col">
            ...
            <div className="px-5 py-3 border-b border-gray-800 bg-gray-900/40 flex flex-wrap items-center gap-4">
            ...
            <div className="flex-1 overflow-auto p-5 grid grid-cols-1 xl:grid-cols-2 gap-4">
              <section className="rounded border border-gray-800 bg-gray-900/40 p-4 flex flex-col gap-3">
                <div className="flex items-center gap-1 border-b border-gray-700 pb-2">
                  <span className="text-xs px-3 py-1 border-b-2 border-emerald-500 text-emerald-300">
                    Snapshot Output
                  </span>
                  {previewData && (
                  ...
                  {previewData && (
                    <div className="ml-auto">
                      <CopyButton getText={() => JSON.stringify(previewData.snapshot, null, 2)} />
                    </div>
                  )}
                </div>
                {previewData ? (
                  <div className="h-[40rem]">
                    <Json5MonacoEditor value={JSON.stringify(previewData.snapshot, null, 2)} onChange={() => {}} readOnly />
                  </div>
                ) : (
              ...
              <section className="rounded border border-gray-800 bg-gray-900/40 p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-1 border-b border-gray-700 pb-2">
                  ...
                  {previewData && (
                    <div className="ml-auto">
                      {previewRightTab === 'decision'
                        ? <CopyButton getText={() => previewData.decision_input} />
                        : <CopyButton getText={() => JSON.stringify(previewData.trigger_payload, null, 2)} />}
                    </div>
                  )}
                </div>
                {/* Editor area */}
                {previewData ? (
                  <div className="h-[40rem]">
                    {previewRightTab === 'decision' ? (
                      <Json5MonacoEditor value={previewData.decision_input} onChange={() => {}} readOnly />
                    ) : (
                      <Json5MonacoEditor value={JSON.stringify(previewData.trigger_payload, null, 2)} onChange={() => {}} readOnly />
                    )}
                  </div>
                ) : (
                  <p className="text-sm text-gray-500">Run the preview to see output here.</p>
                )}
              </section>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6">
          <div className="w-full max-w-6xl max-h-[88vh] bg-gray-950 border border-gray-700 rounded-xl overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-5 grid grid-cols-1 xl:grid-cols-2 gap-4">
              {calcPreviewBlockIndex !== null ? (
                <>
                  {/* Calculation: left — Sources */}
                  <section className="rounded border border-gray-800 bg-gray-900/40 p-4 flex flex-col gap-3">
                    <div className="flex items-center gap-1 border-b border-gray-700 pb-2">
                      <span className="text-xs px-3 py-1 border-b-2 border-emerald-500 text-emerald-300">Sources</span>
                      {form.calculation_blocks[calcPreviewBlockIndex] && (
                        <div className="ml-auto">
                          <CopyButton getText={() => JSON.stringify(form.calculation_blocks[calcPreviewBlockIndex].sources, null, 2)} />
                        </div>
                      )}
                    </div>
                    {form.calculation_blocks[calcPreviewBlockIndex] ? (
                      <div className="h-[40rem]">
                        <Json5MonacoEditor value={JSON.stringify(form.calculation_blocks[calcPreviewBlockIndex].sources, null, 2)} onChange={() => {}} readOnly />
                      </div>
                    ) : <p className="text-sm text-gray-500">No sources configured.</p>}
                  </section>
                  {/* Calculation: right — Result */}
                  <section className="rounded border border-gray-800 bg-gray-900/40 p-4 flex flex-col gap-3">
                    <div className="flex items-center gap-1 border-b border-gray-700 pb-2">
                      <span className="text-xs px-3 py-1 border-b-2 border-emerald-500 text-emerald-300">Calculation Result</span>
                      {calcPreviewData && (
                        <div className="ml-auto">
                          <CopyButton getText={() => JSON.stringify(calcPreviewData.result, null, 2)} />
                        </div>
                      )}
                    </div>
                    {calcPreviewData ? (
                      <div className="h-[40rem]">
                        <Json5MonacoEditor value={JSON.stringify(calcPreviewData.result, null, 2)} onChange={() => {}} readOnly />
                      </div>
                ...
                <>
                  {/* Tool block: left — Raw Output */}
                  <section className="rounded border border-gray-800 bg-gray-900/40 p-4 flex flex-col gap-3">
                    <div className="flex items-center gap-1 border-b border-gray-700 pb-2">
                      <span className="text-xs px-3 py-1 border-b-2 border-emerald-500 text-emerald-300">Raw Output</span>
                      {toolPreviewData && (
                        <div className="ml-auto">
                          <CopyButton getText={() => JSON.stringify(toolPreviewData.raw_output, null, 2)} />
                        </div>
                      )}
                    </div>
                    {toolPreviewData ? (
                      <div className="h-[40rem]">
                        <Json5MonacoEditor value={JSON.stringify(toolPreviewData.raw_output, null, 2)} onChange={() => {}} readOnly />
                      </div>
                    ) : <p className="text-sm text-gray-500">Run the tool preview to see the raw tool output.</p>}
                  </section>
                  {/* Tool block: right — Transformed / Arguments tabs */}
                  <section className="rounded border border-gray-800 bg-gray-900/40 p-4 flex flex-col gap-3">
                    <div className="flex items-center gap-1 border-b border-gray-700 pb-2">
                      <button
                      ...
                      {toolPreviewData && (
                        <div className="ml-auto">
                          {toolRightTab === 'transformed'
                            ? <CopyButton getText={() => JSON.stringify(toolPreviewData.transformed_output, null, 2)} />
                            : <CopyButton getText={() => JSON.stringify(toolPreviewBlockIndex !== null ? form.tool_blocks[toolPreviewBlockIndex]?.arguments ?? {} : {}, null, 2)} />}
                        </div>
                      )}
                    </div>
                    {toolPreviewData ? (
                      <div className="h-[40rem]">
                        {toolRightTab === 'transformed' ? (
                          <Json5MonacoEditor value={JSON.stringify(toolPreviewData.transformed_output, null, 2)} onChange={() => {}} readOnly />
                        ) : (
                          <Json5MonacoEditor value={JSON.stringify(toolPreviewBlockIndex !== null ? form.tool_blocks[toolPreviewBlockIndex]?.arguments ?? {} : {}, null, 2)} onChange={() => {}} readOnly />
                        )}
                      </div>
      ...
      {aiAssistantOpen && (
        <AiAssistantModal
          title="Snapshot Config — AI Assistant"
          contextFile="snapshot_config_assistant.md"
...
export function DecisionPromptConfigEditor() {
  ...
  return (
    ...
    {aiDecisionOpen && (
      <AiAssistantModal
        title="AI Assistant — Decision Prompt"
        contextFile="decision_prompt_assistant.md"
