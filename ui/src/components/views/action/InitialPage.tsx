...
import { api, type InitialConsoleResponse, type SystemUpdateStatusResponse, type InitialConsoleComposerItem } from '@/api/client'
...
const EMPTY: InitialConsoleResponse = {
  ...
  event_composers: { configured_count: 0, enabled_count: 0, items: [] },
  version: {
    local: 'unknown',
    remote: null,
...
export function InitialPage() {
  ...
  const fireAgentTrigger = async (agentId: string) => {
    setTriggerError(null)
    try {
      await api.triggerAgent(agentId)
      setTriggeredId(agentId)
      window.setTimeout(() => setTriggeredId(id => id === agentId ? null : id), 2000)
    } catch (err) {
      setTriggerError(String(err))
    }
  }

  const fireComposerTrigger = async (ecId: string) => {
    setTriggerError(null)
    try {
      await api.triggerComposer(ecId)
      setTriggeredId(ecId)
      window.setTimeout(() => setTriggeredId(id => id === ecId ? null : id), 2000)
    } catch (err) {
      setTriggerError(String(err))
    }
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 text-gray-200">
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        {triggerError && <div className="text-sm text-amber-400">Trigger error: {triggerError}</div>}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-left text-gray-400 border-b border-gray-700">
                  ...
                  <th className="py-2 pr-2">Snapshot Profile</th>
                  <th className="py-2 pr-2">Decision Prompt Profile</th>
                  ...
                  <tr key={agent.agent_id} className="border-b border-gray-800 align-top">
                    ...
                    <td className="py-2 pr-2">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {agent.type === 'AA' && agent.enabled && agent.session_active ? (
                          <button
                            onClick={() => void fireAgentTrigger(agent.agent_id)}
                            title="Fire M5_CANDLE_TRIGGER for this agent"
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded cursor-pointer transition-all ${
                              triggeredId === agent.agent_id
                                ? 'bg-emerald-600/60 text-emerald-100 border border-emerald-400'
                                : `${badge('connected')} hover:bg-emerald-700/50`
                            }`}
                          >
                            <Zap className="w-3 h-3" />
                            {triggeredId === agent.agent_id ? 'triggered' : 'enabled'}
                          </button>
                        ) : (
                        ...
                        {agent.last_error && (
                          <span
                            className="inline-flex items-center gap-0.5 text-red-400 cursor-help"
                            title={`${agent.last_error.event_type} at ${agent.last_error.timestamp}: ${agent.last_error.message}`}
                          >
                    ...
                    <td className="py-2 pr-2 text-gray-300">{agent.snapshot_profile || '-'}</td>
                    <td className="py-2 pr-2 text-gray-300">{agent.decision_prompt_profile || '-'}</td>
                    <td className="py-2 text-gray-400">{agent.comment || ''}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {data.event_composers.items.map((ec: InitialConsoleComposerItem) => (
                  <tr key={ec.ec_id} className="border-b border-gray-800 align-top">
                    ...
                    <td className="py-2 pr-2">
                      {ec.enabled && ec.triggers.includes('m5_candle_trigger') ? (
                        <button
                          onClick={() => void fireComposerTrigger(ec.ec_id)}
                          title="Fire M5_CANDLE_TRIGGER for this EC"
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded cursor-pointer transition-all ${
                            triggeredId === ec.ec_id
                              ? 'bg-emerald-600/60 text-emerald-100 border border-emerald-400'
                              : `${badge('connected')} hover:bg-emerald-700/50`
                          }`}
                        >
                          <Zap className="w-3 h-3" />
                          {triggeredId === ec.ec_id ? 'triggered' : 'enabled'}
                        </button>
                      ) : (
                        <span className={`px-2 py-0.5 rounded ${ec.enabled ? badge('connected') : badge('missing')}`}>
                          {ec.enabled ? 'enabled' : 'disabled'}
                        </span>
                      )}
                    </td>
                    <td className="py-2 pr-2 text-gray-300">{ec.broker || '-'}</td>
                    <td className="py-2 pr-2 text-gray-300">{ec.pair || '-'}</td>
                    <td className="py-2 pr-2 text-gray-300">{ec.triggers.join(', ') || '-'}</td>
                    <td className="py-2 text-gray-400">{ec.comment || ''}</td>
                  </tr>
                ))}
                {data.event_composers.items.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-3 text-gray-500">No Event Composers configured.</td>
                  </tr>
                )}
              </tbody>
            </table>
