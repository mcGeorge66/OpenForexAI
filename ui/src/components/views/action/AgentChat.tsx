  ...
  const executeAgent = async () => {
    ...
    try {
      ...
      const finalMessage: ChatMessage = {
        ...
        content: resultText,
        rawJson: response.final_response || '',
        agentId: response.agent_id,
        timestamp: now(),
        runId: response.run_id,
      }
                ...
                <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`max-w-2xl min-w-0 flex-1 ${msg.role === 'user' ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                    ...
                    >
                      ...
                      <button
                        onClick={e => { e.stopPropagation(); void copyMessage(msg.id, rawJsonMode && msg.rawJson ? msg.rawJson : msg.content) }}
                    ...
                    {/* Body */}
                    {!collapsed && (
                      <div className={`rounded-lg px-3 py-2 text-sm whitespace-pre-wrap break-words w-full select-text ${
                        msg.role === 'user'
                          ? 'bg-emerald-900/50 text-emerald-100'
                          : rawJsonMode && msg.rawJson ? 'bg-gray-900 text-amber-200 font-mono' : 'bg-gray-800 text-gray-200'
                      } ${
                        ...
                        {rawJsonMode && msg.rawJson ? msg.rawJson : msg.content}
                      </div>
                    )}
                  </div>
                </div>
