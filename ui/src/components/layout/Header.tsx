...
function loadOffset(): number {
  ...
  return 0 // will be replaced by ui_utc from config if no localStorage entry exists
}

function hasStoredOffset(): boolean {
  try { return localStorage.getItem(TZ_KEY) !== null } catch { return false }
}
...
export function Header({ lastUpdate, connected }: HeaderProps) {
  ...
  useEffect(() => {
    if (hasStoredOffset()) return
    api.getSystemConfig()
      .then(cfg => {
        const sys = (cfg as Record<string, unknown>)['system'] as Record<string, unknown> | undefined
        const uiUtc = sys?.['ui_utc']
        if (typeof uiUtc === 'number' && uiUtc >= -12 && uiUtc <= 14) {
          setTzOffset(uiUtc)
          setClock(formatClock(new Date(), uiUtc))
        }
      })
