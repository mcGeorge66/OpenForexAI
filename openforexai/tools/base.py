...
(``config/config.json5`` → ``agents.<id>.tool_config``) before calling ``execute()``.
Approval modes:
...
The approval mode is configured per-tool per-agent in ``config/config.json5``.
"""
...
async def bus_request(
    ...
    await bus.publish(msg, triggered_by=context.triggering_message)
