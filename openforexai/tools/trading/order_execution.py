...
async def _get_quote_to_account_rate(
    context: ToolContext,
    quote_ccy: str,
    account_currency: str,
) -> Decimal:
    """Return how many account_currency units equal 1 quote_currency unit.

    Tries two pair candidates in the DataContainer:
    - Direct  quote+account (e.g. EURUSD for quote=EUR, account=USD) → use close as-is
    - Inverse account+quote (e.g. USDJPY for quote=JPY, account=USD) → use 1/close
    Raises RuntimeError if neither pair has candle data.
    """
    direct_pair = quote_ccy + account_currency   # e.g. EURUSD
    candles = await _get_candles_via_bus(context, 1, "M5", pair=direct_pair)
    if candles:
        close = Decimal(str(candles[-1].get("close", "0")))
        if close > 0:
            return close

    inverse_pair = account_currency + quote_ccy  # e.g. USDJPY
    candles = await _get_candles_via_bus(context, 1, "M5", pair=inverse_pair)
    if candles:
        close = Decimal(str(candles[-1].get("close", "0")))
        if close > 0:
    ...
    raise RuntimeError(
        f"Cannot determine cross rate {quote_ccy} → {account_currency}: "
        f"no candle data for {direct_pair} or {inverse_pair}. "
        f"Provide units explicitly."
    )
...
async def _get_account_status_via_bus(context: ToolContext) -> dict:
    ...
    assert context.pair, "context.pair must be set (caller already validated this)"
    pair = context.pair
    response = await bus_request(
        context=context,
...
async def execute_place_order_arguments(
    ...
    if lots_arg is not None and str(lots_arg) != "":
        ...
        # quote_to_account: how many account-currency units = 1 quote-currency unit
        # e.g. USDJPY, account=USD → quote=JPY → rate = 1/150 ≈ 0.00667 USD per JPY
        quote_to_account = Decimal("1")
        if len(pair) >= 6 and account_currency:
            quote_ccy = pair[3:6]
            if quote_ccy != account_currency:
                quote_to_account = await _get_quote_to_account_rate(context, quote_ccy, account_currency)

        equity = Decimal(str(account_data.get("equity", 0)))
        risk_amount = equity * Decimal(str(risk_pct_f / 100.0))
        per_unit_risk = abs(entry_ref - stop_loss) * quote_to_account
        if per_unit_risk <= 0:
    ...
    order_response = await bus_request(
        ...
        payload={"order": order.model_dump(mode="json"), "entry_id": str(entry.id)},
        timeout=30.0,
    )
    ...
    if result_status == "REJECTED":
        broker_detail = order_response.get("close_reason") or ""
        ...
        if broker_detail:
            rejected_updates["close_reasoning"] = broker_detail
        await repo_request(context, "update_order_book_entry", {
            "entry_id": str(entry.id),
            "updates": rejected_updates,
        })
