...
class AgentBridgeTool(BaseTool):
    ...
    async def execute(self, arguments: dict[str, Any], context: ToolContext) -> Any:
        ...
        _bridge_started = _perf()
        await bus.publish(AgentMessage(
            event_type=EventType.BRIDGE_CALL_START,
            source_agent_id=context.agent_id,
            payload={"agent_id": context.agent_id, "tool_name": self.name,
                     "target_agent_id": target_agent_id, "question": question},
        ...
        query_msg = AgentMessage(
            ...
            payload={"question": question, "source": context.agent_id},
        )
        ...
        try:
            ...
            await bus.publish(AgentMessage(
                event_type=EventType.BRIDGE_CALL_END,
                source_agent_id=context.agent_id,
                payload={"agent_id": context.agent_id, "tool_name": self.name,
                         "target_agent_id": target_agent_id, "success": True,
                         "elapsed_ms": round((_perf() - _bridge_started) * 1000, 1),
                         "response": response.get("response", "")[:500]},
            ), triggered_by=context.triggering_message)
            return {
                "response": response.get("response", ""),
                "from_agent": response.get("agent_id", target_agent_id),
            }
        except TimeoutError:
            _log.warning(
                "AgentBridgeTool %r: no response from %r within %.0fs",
                self.name, target_agent_id, self._timeout,
            )
            await bus.publish(AgentMessage(
                event_type=EventType.BRIDGE_CALL_END,
                source_agent_id=context.agent_id,
                payload={"agent_id": context.agent_id, "tool_name": self.name,
                         "target_agent_id": target_agent_id, "success": False,
                         "elapsed_ms": round((_perf() - _bridge_started) * 1000, 1),
