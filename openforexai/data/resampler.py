...
def _bucket_start(
    ...
    Operates purely on wall-clock values and never converts across timezones:
    *ts* is treated as-is, and the result carries *ts*'s original tzinfo
    unchanged (source candles carry the broker's timezone label; this must
    not silently reinterpret it as UTC).
    """
    naive = ts.replace(tzinfo=None)
    shifted = naive - timedelta(minutes=bucket_offset_minutes)
    total_mins = int((shifted - _EPOCH).total_seconds() / 60)
    bucketed_mins = (total_mins // target_mins) * target_mins
    bucket_naive = _EPOCH + timedelta(minutes=bucketed_mins + bucket_offset_minutes)
    return bucket_naive.replace(tzinfo=ts.tzinfo)
