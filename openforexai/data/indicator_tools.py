...
def _values_to_candles(
    values: list[float | None],
    reference_candles: list[Candle],
) -> list[Candle]:
    """Convert a list of float values into minimal Candle objects.

    Uses timestamps and timeframe from reference_candles where available.
    Skips None values (gaps in the series).
    """
    result: list[Candle] = []
    for i, val in enumerate(values):
        ...
        if i < len(reference_candles):
            ts = reference_candles[i].timestamp
            tf = reference_candles[i].timeframe
        else:
            ts = datetime.now(UTC)
            tf = "M5"
...
class IndicatorToolset:
    ...
    async def calculate(
        ...
        if getattr(plugin, "requires_component_pairs", False):
            ...
            pair_candles = await self._dc.get_candles(resolved_broker, pair.upper(), timeframe.upper())
            component_candles: dict[str, list[Candle]] = {}
            for comp_pair in plugin.DXY_COMPONENTS:
                comp = await self._dc.get_candles(resolved_broker, comp_pair, timeframe.upper())
                if comp:
                    component_candles[comp_pair] = comp
            dxy_values = synthetic_dxy(component_candles)
            if not dxy_values:
                return None
            ref_key = next(iter(component_candles), None)
            ref_candles = component_candles[ref_key] if ref_key else []
            dxy_candles = _values_to_candles(dxy_values, ref_candles)
            if not dxy_candles:
                return None
            n = len(dxy_candles)
            plugin._pair_closes = [float(c.close) for c in pair_candles[-n:]] if pair_candles else []
            values = plugin.calculate(dxy_candles, period, min(max(history, 1), len(dxy_candles)))
            if not values:
                return None
