...
def _apply_event_log_tz() -> None:
    from openforexai.messaging.event_log import configure as _el_configure
    system_cfg = _system_config.get("system", {}) if isinstance(_system_config, dict) else {}
    offset = int(system_cfg.get("ui_utc", 0))
    _el_configure(offset)
...
async def _execute_agent_inspection(
    *,
    ...
    try:
        _base_prompt = agent._system_prompt
...
class LLMCheckerRequest(BaseModel):
    ...
    reasoning_effort: str | None = Field(
        default=None,
        description="Reasoning effort level for supported models (low / medium / high). Ignored by providers that do not support it.",
    )
...
@router.get("/chartshots/{filename}")
async def get_chartshot_image(filename: str):  # type: ignore[return]
    """Serve a chartshot PNG from the configured chartshot output_dir."""
    import re as _re
    from fastapi.responses import FileResponse as _FileResponse
    if not _re.fullmatch(r"[A-Za-z0-9_.\-]+\.png", filename, _re.IGNORECASE):
    ...
    shots_dir = (root / output_dir).resolve()
    target = (shots_dir / filename).resolve()
    if not str(target).startswith(str(shots_dir)):
        raise HTTPException(status_code=400, detail="Path traversal not allowed")
    if not target.exists():
        raise HTTPException(status_code=404, detail=f"Chartshot not found: {filename}")
    return _FileResponse(str(target), media_type="image/png")


@router.delete("/chartshots/{filename}")
async def delete_chartshot_image(filename: str):
    """Delete a chartshot PNG (used by the UI after preview display)."""
    import re as _re
    if not _re.fullmatch(r"[A-Za-z0-9_.\-]+\.png", filename, _re.IGNORECASE):
        raise HTTPException(status_code=400, detail="Invalid chartshot filename")
    root = _project_root()
...
async def get_console_initial() -> dict[str, Any]:
    ...
    if isinstance(agents_cfg, dict):
        for agent_id in sorted(agents_cfg.keys()):
            ...
            # Level 3: last active timestamp + trigger-aware stale detection.
            last_active: datetime | None = _monitoring_bus.agent_last_active(agent_id) if _monitoring_bus is not None else None
            is_stale, stale_reason = _compute_agent_stale(cfg, agent_id, now_utc)
            agents.append({
                "agent_id": agent_id,
                "enabled": enabled,
                "type": cfg.get("type"),
...
async def list_agents() -> list[AgentInfo]:
    ...
    registered = set(_bus.registered_agents()) if _bus else set()
    result = []
    for aid in sorted(enabled_agent_ids):
        q = _bus._agent_queues.get(aid) if _bus and aid in registered else None
...
async def _trigger_m5_candle(broker_module_name: str | None, pair: str | None, label: str, *, force_run: bool = False) -> TriggerResponse:
    if _data_container is None:
        raise HTTPException(status_code=503, detail="DataContainer not available")
    if not pair or not broker_module_name:
...
async def trigger_agent(agent_id: str) -> TriggerResponse:
    ...
    if agent_cfg.get("type") != "AA":
        raise HTTPException(status_code=400, detail=f"Agent {agent_id!r} is not an AA agent")
    return await _trigger_m5_candle(agent_cfg.get("broker"), agent_cfg.get("pair"), agent_id, force_run=True)
...
async def trigger_composer(ec_id: str) -> TriggerResponse:
    ...
    return await _trigger_m5_candle(ec_cfg.get("broker"), ec_cfg.get("pair"), ec_id, force_run=True)
...
async def _trigger_ec_config_refresh() -> dict[str, int]:
    ...
    eligible_ecs = [
        ec_id
        for ec_id, cfg in _system_config.get("event_composers", {}).items()
        if isinstance(cfg, dict) and cfg.get("enable", True) and ec_id in registered
    ]

    requested = 0
    for ec_id in eligible_ecs:
        await _bus.publish(AgentMessage(
...
async def _run_runtime_agent(agent) -> None:
    ...
    try:
        ...
        _logger.exception("Runtime agent %r crashed on startup", agent_id)
        if _monitoring_bus is not None:
            try:
                from openforexai.models.monitoring import MonitoringEvent, MonitoringEventType
                _monitoring_bus.emit(MonitoringEvent(
                    ...
                    payload={
                        ...
                        "context": "runtime_agent_startup_failed",
                    },
                ))
            except Exception:
                pass
        # A crashed start() must not leave the agent registered as "active" —
        # otherwise it silently manages nothing forever. Unregister so a
        # subsequent config save's to_start check will retry it.
        _active_agents.pop(agent_id, None)
        _runtime_agents.pop(agent_id, None)
        _runtime_agent_tasks.pop(agent_id, None)
        if _bus is not None:
            try:
...
async def _run_runtime_ec(ec) -> None:
    ec_id = getattr(ec, "ec_id", "?")
    try:
        await ec.start()
    except asyncio.CancelledError:
        raise
    except Exception as exc:
        _logger.exception("Runtime EventComposer %r crashed on startup", ec_id)
        if _monitoring_bus is not None:
            try:
                from openforexai.models.monitoring import MonitoringEvent, MonitoringEventType
                _monitoring_bus.emit(MonitoringEvent(
                    ...
                    payload={
                        ...
                        "error": str(exc),
                        "error_type": type(exc).__name__,
                        "context": "runtime_ec_startup_failed",
                    },
                ))
            except Exception:
                pass
        ...
        _runtime_composer_tasks.pop(ec_id, None)
        if _bus is not None:
            try:
...
async def _apply_runtime_composer_changes(previous_system_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if _bus is None:
        return {"started": 0, "stopped": 0, "refresh": {"requested": 0, "eligible": 0, "registered": 0}}
    ...
    to_stop = sorted(
        ...
        if ec_id not in next_enabled and ec_id in _active_composers
    )

    stopped = 0
    for ec_id in to_stop:
        ec = _active_composers.pop(ec_id, None)
        _runtime_composers.pop(ec_id, None)
        if ec is not None:
            await ec.stop()
    ...
    refresh = await _trigger_ec_config_refresh()
...
async def _apply_runtime_agent_changes(previous_system_config: dict[str, Any]) -> dict[str, Any]:
    ...
    to_stop = sorted(
        ...
        if agent_id not in next_enabled and agent_id in _active_agents
    )

    stopped = 0
    for agent_id in to_stop:
        agent = _active_agents.pop(agent_id, None)
        _runtime_agents.pop(agent_id, None)
        if agent is not None:
            await agent.stop()
...
_EVENT_SCHEMAS_PATH_REL = Path("config") / "event_schemas.json5"
...
async def save_config_file(name: str, content: dict[str, Any] | str) -> dict:
    ...
    if name == "agent_tools" and _tool_registry is not None:
        try:
            ...
            from openforexai.tools.system.agent_bridge import AgentBridgeTool, register_bridge_tools_from_config

            old_bridge_names = [t.name for t in _tool_registry.all_tools() if isinstance(t, AgentBridgeTool)]
            for tool_name in old_bridge_names:
                _tool_registry.unregister(tool_name)

            agent_tool_cfg = AgentToolConfig.load(cfg_path)
            bridge_count = register_bridge_tools_from_config(agent_tool_cfg.raw_bridge_tools(), _tool_registry)
            extra["bridge_tools_reloaded"] = bridge_count
        except Exception as exc:
            extra["bridge_tools_reload_error"] = str(exc)

    return {"status": "saved", "file": f"config/RunTime/{name}.json5", **extra}


# ── Event Schemas ──────────────────────────────────────────────────────────────

@router.get("/config/event_schemas")
async def get_event_schemas() -> dict:
    """Return the event_schemas.json5 config (all event types and required fields)."""
    path = _project_root() / _EVENT_SCHEMAS_PATH_REL
    if not path.exists():
        return {}
    try:
...
@router.put("/config/event_schemas")
async def save_event_schemas(content: dict[str, Any] | str) -> dict:
    """Save event_schemas.json5 and hot-reload the bus schema cache."""
    path = _project_root() / _EVENT_SCHEMAS_PATH_REL
    _write_json_file(path, content)
    extra: dict[str, Any] = {}
    if _bus is not None:
        try:
            _bus.reload_event_schemas()
            extra["schemas_reloaded"] = True
            extra["schema_count"] = len(_bus._event_schemas)
        except Exception as exc:
            extra["schemas_reload_error"] = str(exc)
    return {"status": "saved", "file": str(_EVENT_SCHEMAS_PATH_REL), **extra}


@router.post("/config/reload/event_schemas")
async def reload_event_schemas() -> dict:
    """Hot-reload event_schemas.json5 without saving."""
    if _bus is None:
        raise HTTPException(status_code=503, detail="EventBus not available")
    _bus.reload_event_schemas()
    return {"status": "ok", "schema_count": len(_bus._event_schemas)}
...
async def list_events(
    ...
    """Query the persistent event log (newest-first, payloads decompressed)."""
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Event log read error: {exc}")


@router.get("/events/{event_id}")
async def get_event(event_id: str) -> dict:
    """Return a single event by ID (payload decompressed)."""
    from openforexai.messaging.event_log import read_event
    ev = read_event(event_id)
    if ev is None:
...
async def get_event_trace(event_id: str) -> list[dict]:
    """Return the full event chain: root first, target last."""
    import traceback as _tb
    from openforexai.messaging.event_log import read_trace
    try:
        result = read_trace(event_id)
    except Exception as _exc:
        _detail = f"{type(_exc).__name__}: {_exc}\n{_tb.format_exc()}"
        raise HTTPException(status_code=500, detail=_detail)
    if not result:
        raise HTTPException(status_code=404, detail=f"Event {event_id!r} not found in event log")
    return result
...
class LLMAssistantChatRequest(BaseModel):
    ...
    script: str = ""                          # current script shown in the editor (optional)
    question: str
    history: list[LLMAssistantMessage] = Field(default_factory=list)
    context_data: str | None = Field(         # generic context (JSON, config, etc.) — not Python
        default=None,
...
def _resolve_file_refs(text: str) -> str:
    """Replace [[relative/path]] references in context text with file contents.

    Only files within the project root are allowed (path-traversal safe).
    Missing or unreadable files are left as-is in the text.
    """
    import re
    ...
    def _replace(match: re.Match) -> str:  # type: ignore[type-arg]
        ...
        try:
            ...
            target.relative_to(root)  # raises ValueError if outside project root
...
class ScriptValidateRequest(BaseModel):
    code: str = Field(..., description="Python script source code to validate")

class ScriptValidateError(BaseModel):
    line: int
    column: int
    message: str
...
class ScriptValidateResponse(BaseModel):
    valid: bool
    errors: list[ScriptValidateError] = Field(default_factory=list)


@router.post("/scripts/validate", response_model=ScriptValidateResponse)
async def validate_script(req: ScriptValidateRequest) -> ScriptValidateResponse:
    """Parse a Python script and return syntax errors (line, column, message)."""
    import ast
    try:
        ast.parse(req.code)
        return ScriptValidateResponse(valid=True)
    except SyntaxError as exc:
...
def build_app(
    ...
    if _ui_dist.exists():
        ...
        async def _serve_spa(full_path: str) -> _FileResponse:  # type: ignore[return]
            ...
            try:
                target.relative_to(_ui_dist_resolved)
            except ValueError:
