...
class AbstractRepository(ABC):
    ...
    async def kb_list_documents(self) -> list[dict]:
        """Return all documents (metadata only, no content)."""
        return []

    async def kb_get_document(self, doc_id: str) -> dict | None:
        """Return a single document including content, or None."""
        return None

    async def kb_create_document(self, doc: dict) -> str:
        """Create a document/folder. Returns the new id."""
        return ""
    ...
    async def kb_delete_document(self, doc_id: str) -> None:
        """Delete a document and its FTS entry."""
        pass

    async def kb_search(self, query: str, limit: int = 20) -> list[dict]:
        """Full-text search. Returns list of {id, title, snippet}."""
        return []
