...
_SCHEMA_PATH = Path("config/event_schemas.json5")
...
class EventBus:
    ...
    def __init__(
        ...
        # Event schemas: event_type → {"required": [...]}
        self._event_schemas: dict[str, dict] = {}
        self._load_event_schemas()

    # ── Event-schema management ───────────────────────────────────────────────

    def _load_event_schemas(self) -> None:
        try:
            raw = json5.loads(_SCHEMA_PATH.read_text(encoding="utf-8"))
            self._event_schemas = {k: v for k, v in raw.items() if not k.startswith("_")}
            _log.info("Event schemas loaded: %d types", len(self._event_schemas))
        except Exception as exc:
            _log.error("Failed to load event_schemas.json5: %s — payload validation disabled", exc)
            self._event_schemas = {}

    def reload_event_schemas(self) -> None:
        """Hot-reload event_schemas.json5 from disk."""
        self._load_event_schemas()
        self._emit_monitoring("eventbus", "EVENT_SCHEMAS_RELOADED",
                              schema_count=len(self._event_schemas))

    def _validate_message(self, message: AgentMessage) -> None:
        if not message.source_agent_id:
            raise ValueError(
                f"AgentMessage.source_agent_id must not be empty (event_type={message.event_type})"
            )
        event_key = str(message.event_type)
        schema = self._event_schemas.get(event_key)
        if schema is None:
            _log.warning("No schema defined for EventType %r — payload not validated", event_key)
            return
        ...
        if missing:
            ...
            try:
                payload_repr = json.dumps(message.payload, default=str, ensure_ascii=False)
            except Exception:
                payload_repr = repr(message.payload)
            raise ValueError(
                f"Event {event_key!r} missing required payload fields: {missing} "
                f"| timestamp={ts} "
                f"| source={message.source_agent_id!r} "
                f"| instrument={message.instrument!r} "
                f"| payload={payload_repr}"
            )

    def _is_trace_root(self, event_type: str) -> bool:
        schema = self._event_schemas.get(str(event_type))
        return bool(schema and schema.get("trace_root", False))
    ...
    async def publish(
        ...
        *,
        triggered_by: AgentMessage | None = None,
    ) -> None:
        """Validate, chain, persist, and enqueue *message* for routing and delivery.
        ...
        triggered_by:
            The upstream AgentMessage that caused this publish.  The bus will
            automatically extend message.chain with triggered_by's chain + its id.
        """
        # Build chain from triggering message
        if triggered_by is not None:
            message = message.model_copy(update={
                "chain": triggered_by.chain + [triggered_by.id]
            })

        # Validate envelope and payload against schema
        self._validate_message(message)

        # Persist asynchronously — never blocks dispatch
        asyncio.ensure_future(self._persist_event(message, self._is_trace_root(str(message.event_type))))

        await self._inbound.put(message)

    async def _persist_event(self, message: AgentMessage, is_trace_root: bool = False) -> None:
        try:
            from openforexai.messaging.event_log import persist_event
            await persist_event(message, is_trace_root=is_trace_root)
        except Exception as exc:
            _log.error("Failed to persist event %s: %s", message.id, exc)
