...
class RoutingRule:
    ...
    def matches_sender(self, sender_id: str) -> bool:
        ...
        if aid is None:
            # Infrastructure senders (non-structured IDs, e.g. "llm:xyz") can only
            # match rules with from="*" — and that case already returned True above.
            return False
...
class RoutingTable:
    ...
    def resolve(
        ...
        """Return ``(target_member_ids, matched_any)``.

        *target_member_ids* — deduplicated list of registered member IDs that
            should receive this message (queue-based delivery).

        *matched_any* — True if at least one routing rule matched.

        If no rule matches, returns ``([], False)`` — the bus will log a
        MonitoringBus warning and discard the message.

        Note: ``"@handlers"`` is no longer a valid routing target and will be
        skipped with a warning if encountered.
        """
        ...
        for rule in self._rules:  # already sorted by priority
            ...
            if rule.to.strip() == "@handlers":
                _log.warning(
                    "Rule %r uses deprecated '@handlers' target — skipping. "
                    "Convert to explicit member ID.",
                    rule.id,
                )
            else:
        ...
        return list(targets.keys()), matched_any
