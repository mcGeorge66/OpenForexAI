...
def _inject_images_anthropic(
    ...
    """Prepend image blocks to the last user message in *messages*."""
    if not resolved:
        return messages
    ...
    for i in range(len(augmented) - 1, -1, -1):
        if augmented[i].get("role") == "user":
            ...
            for img in resolved:
                header, _, b64 = img.partition(";base64,")
                mt = header.split("data:")[-1] if "data:" in header else "image/png"
        ...
        async def _call() -> LLMResponse:
            if resolved_images:
                ...
                for img in resolved_images:
                    header, _, b64_data = img.partition(";base64,")
                    media_type = header.split("data:")[-1] if "data:" in header else "image/png"
                    user_content.append({
                        "type": "image",
                        "source": {
                            "type": "base64",
