...
def _inject_images_openai(
    ...
    """Prepend image blocks to the last user message for OpenAI/Azure format."""
    if not resolved:
        return messages
...
class AzureOpenAILLMProvider(AbstractLLMProvider):
    ...
    async def complete(
        ...
        def _on_complete_attempt_error(
            attempt: int, total: int, elapsed_ms: float, exc: Exception
        ) -> None:
    ...
    async def complete_with_tools(
        ...
        def _on_fallback_attempt_error(
            ...
            detail = describe_exception(fallback_exc)
            self._emit_debug_diagnostic(
                "llm_http_attempt_failed",
                operation="complete_with_tools",
                call_mode="without_tools_fallback",
    ...
    def _sanitize_messages(cls, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Convert a list of canonical messages to Azure/OpenAI format."""
