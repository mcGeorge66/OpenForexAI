...
class PostgreSQLRepository(AbstractRepository):
    ...
    async def _ensure_order_book_table(self) -> None:
        await self._execute(
            ...
            CREATE TABLE IF NOT EXISTS order_book_entries (
                ...
                confirmed_by_broker    BOOLEAN NOT NULL DEFAULT FALSE
            )
            """
        )
        await self._execute("ALTER TABLE order_book_entries ADD COLUMN IF NOT EXISTS sync_key TEXT")
        await self._execute("ALTER TABLE order_book_entries ADD COLUMN IF NOT EXISTS close_requested_at TEXT")
        await self._execute(
            "ALTER TABLE order_book_entries ADD COLUMN IF NOT EXISTS confirmed_by_broker BOOLEAN NOT NULL DEFAULT FALSE"
        )
    ...
    async def save_order_book_entry(self, entry: OrderBookEntry) -> str:
        ...
        await self._execute(
            ...
            ON CONFLICT (id) DO UPDATE SET
                ...
                confirmed_by_broker=EXCLUDED.confirmed_by_broker
            """,
            eid,
            entry.broker_name,
            ...
            bool(entry.confirmed_by_broker),
        )
        return eid
    ...
    async def _row_to_order_book_entry(self, row: dict[str, Any]) -> OrderBookEntry:
        ...
        return OrderBookEntry(
            ...
            close_requested_at=(
                datetime.fromisoformat(row["close_requested_at"]) if row.get("close_requested_at") else None
            ),
            ...
            confirmed_by_broker=bool(row["confirmed_by_broker"]) if row.get("confirmed_by_broker") is not None else False,
        )
