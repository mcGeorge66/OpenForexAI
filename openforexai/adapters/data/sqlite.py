...
class SQLiteDataContainer(SQLiteRepository, AbstractDataContainer):
    ...
    async def save_llm_conversation(
        ...
        """Upsert the complete LLM messages list for one trading cycle (zlib-compressed)."""
        now = datetime.now(UTC).isoformat()
        start = (started_at or datetime.now(UTC)).isoformat()
        record_id = str(uuid.uuid4())
        raw = json.dumps(messages, default=str, ensure_ascii=False).encode("utf-8")
        compressed = zlib.compress(raw, level=6)
        await self._db().execute(
            """
            INSERT INTO agent_conversations
                (id, agent_id, session_id, messages, turn_count, started_at, updated_at, is_compressed)
            VALUES (?,?,?,?,?,?,?,?)
    ...
    async def get_last_llm_conversation(
        ...
        cursor = await self._db().execute(
            """
            SELECT messages, is_compressed FROM agent_conversations
            WHERE agent_id=?
        ...
        try:
            ...
            is_compressed = (row["is_compressed"] if isinstance(row, dict) else row[1]) if len(row) > 1 else 0
            if is_compressed and isinstance(raw, (bytes, bytearray)):
                raw = zlib.decompress(raw).decode("utf-8")
            elif isinstance(raw, (bytes, bytearray)):
                raw = raw.decode("utf-8")
            return json.loads(raw)
        except (json.JSONDecodeError, KeyError, TypeError, zlib.error):
            return None
