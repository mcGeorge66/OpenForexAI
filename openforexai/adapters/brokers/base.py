...
# Grace window (seconds) before a broker-side close/orphan can be trusted.
# Protects against declaring a just-opened position "closed" (or a pending
# order "orphaned") purely because it raced a broker positions snapshot that
# was fetched a moment before the order finished placing.
_SYNC_GRACE_SECONDS = 120
...
def normalize_candle(raw: dict[str, Any], timeframe: str) -> Candle:
    ...
    Expected raw keys (flexible — supports both long and short names):
        ...
        spread             → raw price delta as supplied by the caller
                              (unit is caller-defined; defaults to 0 if absent)
    """
...
class BrokerBase(AbstractBroker):
    ...
    async def _handle_close_request(self, msg: AgentMessage, source_id: str, event_bus) -> None:
        ...
        try:
            ...
            if hasattr(result, "model_dump"):
                ...
                result_dict = {"result": str(result), "success": True}
            result_dict["error"] = None
        except Exception as exc:
            _log.error("Broker: close_position failed: %s", exc)
    ...
    async def _handle_modify_request(self, msg: AgentMessage, source_id: str, event_bus) -> None:
        ...
        try:
            ...
            _log.error("Broker: modify_position failed: %s", exc)
            result_dict = {"success": False, "error": str(exc)}

        # ORDER_MODIFY_RESULT schema requires entry_id — echo it from the request
        # so the response passes bus validation and reaches the caller.
        result_dict.setdefault("entry_id", payload.get("entry_id"))
    ...
    async def _handle_candle_repair_request(self, msg: AgentMessage, source_id: str, event_bus) -> None:
        pair = msg.instrument or ""
        count = int(msg.payload.get("count", 200))
        broker_name = msg.payload.get("broker_name", self.short_name)
        try:
        ...
        await event_bus.publish(
            AgentMessage(
                ...
                source_agent_id=source_id,
                target_agent_id=msg.source_agent_id,
                instrument=pair or None,
                payload={"broker_name": broker_name, "candles": candle_dicts, "error": error},
                correlation_id=str(msg.id),
            ),
            triggered_by=msg,
        )
    ...
    @staticmethod
    def _past_sync_grace_period(entry: dict, now: datetime) -> bool:
        """True once *entry* is old enough that a broker-side discrepancy can be trusted.

        Uses ``requested_at`` (when the local entry was created) so a position
        that just finished placing — and hasn't shown up in the broker
        positions snapshot yet — is not mistaken for a broker-side close.
        """
        ts_raw = entry.get("requested_at")
        if not ts_raw:
            return True
        try:
            rt = datetime.fromisoformat(str(ts_raw).replace("Z", "+00:00"))
        except Exception:
            return True
        return (now - rt).total_seconds() > _SYNC_GRACE_SECONDS
    ...
    async def _sync_pair(
        ...
        *,
        ...
        """Reconcile local order-book entries against live broker positions for one pair.

        Shared by the periodic ``_sync_loop`` and the manual ``trigger_sync`` tool
        so both paths apply the identical reconciliation (including SL/TP refresh)
        and, when ``emit_events`` is True, report discrepancies identically.

        Broker positions and local entries are fetched back-to-back (no other
        pair's processing or awaits in between) to minimise the window in which
        an agent opens or closes a position concurrently with this sync pass —
        a wider window here previously caused just-opened positions to be
        falsely marked closed, and just-closed positions to be ghost-imported
        as new open entries.

        Returns ``(discrepancies, local_open_count, broker_positions_count)`` —
        *discrepancies* is a list of dicts ``{entry_id, pair, direction, close_reason}``.
        """
        source = f"broker.{self.short_name}"
        ...
        for entry in local_open:
            ...
            if entry.get("broker_order_id") and entry["broker_order_id"] not in broker_ids:
                # Grace period: this entry might have just finished placing after
                # the broker_positions snapshot above was taken — don't declare
                # it closed yet, the next sync pass will re-check it.
                if not self._past_sync_grace_period(entry, now):
                    continue
                # Re-verify with a fresh broker read before finalising the close:
                # positions matched/updated above each involved a repo round-trip,
                # widening the window since broker_ids was computed.
                reverify_positions = await self.get_open_positions()
                if any(p.broker_position_id == entry["broker_order_id"] for p in reverify_positions):
                    continue
                ...
                if self._past_sync_grace_period(entry, now):
        ...
        return found, len(local_open), len(broker_ids)
        ...
        while self._running:
            ...
            try:
                for pair in sorted(self._pairs):
                    ...
                    discrepancies, local_open_count, broker_positions_count = await self._sync_pair(
                        pair, event_bus, source_agent_id,
                        emit_events=True,
                    )

                    self._emit(source, MonitoringEventType.SYNC_CHECK_COMPLETED,
                               broker_name=self.short_name, pair=pair,
                               positions_at_broker=broker_positions_count,
    ...
    async def trigger_sync(
        ...
        """
        source_agent_id = _adapter_agent_id(self.short_name, pair)
        found, _local_open_count, _broker_positions_count = await self._sync_pair(
            pair, event_bus, source_agent_id,
            emit_events=True,
        )
        return found
