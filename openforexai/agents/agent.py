...
from openforexai.tools.argument_templates import build_agent_placeholder_values, resolve_argument_templates
...
class Agent:
    ...
    def __init__(
        ...
        self._running = False
        # Set by stop(); checked before entering the run phase so a stop() issued
        # while start() is still waiting for config is never silently overridden.
        self._stop_requested = False
        # Task references for the timer/message loops, so stop() can cancel them
        # directly instead of relying on the loops noticing a flipped bool — this
        # is what actually interrupts a multi-minute asyncio.sleep() mid-wait.
    ...
    async def start(self) -> None:
        ...
        # A stop() issued while we were still waiting for config (Phase 2) or
        # applying it (Phase 3) must not be silently overridden by Phase 4.
        if self._stop_requested:
            self._logger.info("stop() was called during startup — aborting before run phase")
            return
        ...
        try:
            async with asyncio.TaskGroup() as tg:
                if timer_cfg.get("enabled"):
                    self._loop_tasks.append(tg.create_task(
    ...
    async def stop(self) -> None:
        """Signal the agent to stop and cancel its running loops.

        Safe to call at any point: before start() has finished the config
        bootstrap (guards Phase 4 from ever running), or while the timer/message
        loops are active — task cancellation interrupts a multi-minute timer
        sleep or a paused wait_until_resumed() immediately, instead of waiting
        for the loop to next check a flag.
        """
        self._running = False
    ...
    def _apply_config(self, payload: dict[str, Any]) -> None:
        ...
        _ph = build_agent_placeholder_values(
            agent_id=self.agent_id,
            agent_config=cfg,
            broker_name=runtime_broker_name,
        )
        self._system_prompt = resolve_argument_templates(self._system_prompt, _ph)
    ...
    async def _run_timer_loop(self, interval: int) -> None:
        ...
        while self._running:
            ...
            if self._run_lock.locked():
                # A message-loop-triggered (or agent_query) cycle is already
                # running — skip this tick rather than run concurrently against
                # the shared ToolContext; the next tick will retry.
                self._logger.info("Timer trigger skipped — cycle already in progress", agent_id=self.agent_id)
                try:
                    self._monitoring_bus.emit(MonitoringEvent(
                        timestamp=datetime.now(UTC),
                        source_module=f"agent:{self.agent_id}",
        ...
        while self._running:
            ...
            try:
                ...
                if event_val == EventType.AGENT_QUERY.value:
                    ...
                    force_run = isinstance(msg.payload, dict) and msg.payload.get("force_run") is True
                    if force_run:
                        should_run = True
                        trigger_count = None
                        self._m5_candle_event_count = 0
    ...
    async def _run_cycle(
        ...
        _cycle_started = perf_counter()
        context = self._tool_dispatcher._context if self._tool_dispatcher else None
        await self._bus.publish(AgentMessage(
            event_type=EventType.AGENT_CYCLE_START,
            source_agent_id=self.agent_id,
            instrument=context.pair if context else None,
            payload={
                "agent_id": self.agent_id,
                "trigger": trigger,
                "source": source,
        ...
        try:
            await self._run_cycle_inner(
                ...
                cycle_started_perf=_cycle_started,
            )
            await self._bus.publish(AgentMessage(
                event_type=EventType.AGENT_CYCLE_END,
                source_agent_id=self.agent_id,
                payload={
                    "agent_id": self.agent_id,
                    "trigger": trigger,
                    "success": True,
                    "elapsed_ms": round((perf_counter() - _cycle_started) * 1000, 1),
                },
            ), triggered_by=triggering_msg)
    ...
    async def _run_cycle_inner(
        ...
        correlation_id: str | None = None,
        triggering_msg=None,
        cycle_started_perf: float = 0.0,
    ) -> None:
        """Inner implementation of one agent decision cycle."""
        if runtime_control.is_paused():
        ...
        cycle_pair = self._resolve_cycle_pair(trigger, payload, triggering_msg)
        ...
        if context is not None:
            context.triggering_message = triggering_msg

        # Build the user message for this cycle
        cycle_started = cycle_started_perf or perf_counter()
        try:
            ...
            if decision_snapshot is None and self._fallback_snapshot_profile_config and context is not None and cycle_pair:
                try:
                    fallback_snapshot, fallback_errors = await build_analysis_snapshot(
                        broker_name=context.broker_name,
                        pair=cycle_pair,
                        trigger_payload={},
                    ...
                    if fallback_errors:
                        self._logger.warning(
                            "Decision prompt fallback snapshot has validation errors",
                            agent_id=self.agent_id,
    ...
    def _resolve_cycle_pair(self, trigger: str, payload: dict[str, Any], triggering_msg=None) -> str | None:
        ...
        # Primary: instrument field on the triggering message
        if triggering_msg is not None and triggering_msg.instrument:
            return triggering_msg.instrument.strip().upper()
        ...
        if isinstance(trigger_payload, dict):
            instrument = trigger_payload.get("instrument") or trigger_payload.get("pair")
    ...
    async def _execute_broker_decision_fallback(
        ...
        *,
        ...
        order_direction_raw = str(decision.get("direction_sent", "")).strip().lower()
        if order_direction_raw not in {"buy", "sell"}:
            self._logger.warning(
                "Broker decision fallback: action_taken=AUTO_PLACE_ORDER but direction_sent "
                "is missing/invalid — order NOT placed",
                agent_id=self.agent_id,
                direction_sent=decision.get("direction_sent"),
            )
            self._emit_system_error(
                "Broker decision fallback: action_taken=AUTO_PLACE_ORDER but direction_sent "
                f"is missing/invalid ({decision.get('direction_sent')!r}) — order NOT placed."
            )
            return
        ...
        if isinstance(reason, str) and reason.strip():
            arguments["reasoning"] = reason.strip()
