...
Usage::
    ...
    cfg = load_json_config(Path("config/system.json5"))
    module_cfg = load_json_config(Path("config/modules/llm/anthropic_claude.json5"))
"""
...
def load_json_config(path: str | Path) -> dict[str, Any]:
    ...
    Raises ``ValueError`` if the file contains invalid JSON5.
    """
    data = json5.loads(Path(path).read_text(encoding="utf-8"))
