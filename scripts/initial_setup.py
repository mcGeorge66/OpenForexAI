...
def _materialize_configs(
    ...
    for info in selected:
        ...
        rendered = _render_module_config_from_sample(info.sample_path, static_values)
        if target.exists() and not Confirm.ask(f"Overwrite existing {target}?", default=False):
            console.print(f"[yellow]Skipped[/yellow] {target}")
        else:
            target.write_text(rendered, encoding="utf-8")
            console.print(f"[green]Written[/green] {target}")
...
def _write_system_config(selected_llm: dict[str, str], selected_broker: dict[str, str]) -> None:
    ...
    if SYSTEM_CFG.exists():
        try:
            data = json5.loads(SYSTEM_CFG.read_text(encoding="utf-8"))
        except Exception as exc:
            raise RuntimeError(
                f"Refusing to overwrite {SYSTEM_CFG}: it exists but could not be parsed ({exc}). "
                "Fix or remove the file manually before re-running setup."
            )
        if not isinstance(data, dict):
            raise RuntimeError(f"Refusing to overwrite {SYSTEM_CFG}: top-level value is not an object.")
        modules = data.get("modules")
        if not isinstance(modules, dict):
            modules = {}
            data["modules"] = modules
        modules["llm"] = selected_llm
        modules["broker"] = selected_broker
        # Re-serializing loses JSON5 comments — keep a timestamped backup of the
        # original so nothing (comments included) is unrecoverable.
        backup = SYSTEM_CFG.with_name(f"{SYSTEM_CFG.name}.bak-{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        shutil.copy2(SYSTEM_CFG, backup)
        SYSTEM_CFG.write_text(json.dumps(data, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        console.print(f"[yellow]Backup written:[/yellow] {backup}")
        console.print(f"[green]Updated modules in[/green] {SYSTEM_CFG} [dim](agents/profiles/composers preserved)[/dim]")
        return

    stub = {"modules": {"llm": selected_llm, "broker": selected_broker}}
    SYSTEM_CFG.write_text(json.dumps(stub, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
...
def _ask_for_env_values(
    ...
    for key in sorted(required.keys()):
        ...
        value = questionary.password(msg).ask()
        value = (value or "").strip()
