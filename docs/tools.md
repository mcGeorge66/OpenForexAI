...
- [github-updater.py - Release Updater](#github-updaterpy---release-updater)
- [openforexai-wrapper.py - Restart Wrapper](#openforexai-wrapperpy---restart-wrapper)
...
| [`github-updater.py`](#github-updaterpy---release-updater) | GitHub release updater (files + deps + UI build) |
| [`openforexai-wrapper.py`](#openforexai-wrapperpy---restart-wrapper) | Wrapper supervisor for restart signaling |
...
## github-updater.py - Release Updater

`github-updater.py` updates the installation from GitHub releases (including prereleases), applies release files, installs dependencies, and rebuilds the UI.

### Usage

```bash
python tools/github-updater.py
python tools/github-updater.py --yes
python tools/github-updater.py --version 0.6.0
```

### Behavior

- Reads local version from `config/config.default.json5`
- Fetches latest release version from GitHub API
- Applies release archive files while preserving runtime/user config paths
- Runs:
  - `pip install -e ".[all]"`
  - `npm install --prefix ui`
  - `npm run build --prefix ui`

After update, restart OpenForexAI.

---

## openforexai-wrapper.py - Restart Wrapper

`openforexai-wrapper.py` runs OpenForexAI as a supervised child process and enables in-place restart signaling.
...
- Starts `python -m openforexai.main` as child
- Exposes restart signal path via environment variables
- Watches `.runtime/restart.requested`
- On restart signal:
  - terminates child
  - relaunches child in same wrapper session

This is the recommended mode when using the UI `Restart now` button.
...
- `audit*.jsonl` (monitor/audit outputs, usually ignored in git)
