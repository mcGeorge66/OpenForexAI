...
1. Bootstrap
   ...
   └── Create Agent(s) from config.json5 definitions
...
The E2E test uses the real `config.json5` with mock adapters injected at bootstrap time:

```python
@pytest.fixture
async def full_system():
    config = load_json_config(Path("config/config.json5"))
