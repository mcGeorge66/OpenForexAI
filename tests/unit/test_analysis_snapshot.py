...
def _candle_to_response_dict(c: Candle) -> dict[str, object]:
    """Mirror DataContainer._on_candles_request's exact wire format (OHLC as strings)."""
    return {
...
class _FakeCandleBus:
    """Minimal EventBus double that answers CANDLES_REQUEST like DataContainer does.

    Tools now fetch candles via ``bus_request(context, CANDLES_REQUEST, ...)``
    instead of a directly-injected ``data_container`` object (that kwarg was
    removed from ToolContext / build_analysis_snapshot / preview_snapshot_tool_block).
    This double implements exactly the three EventBus methods bus_request() calls:
    register_response_future, publish, cancel_response_future.
    """

    def __init__(self, candles_by_timeframe: dict[str, list[Candle]]) -> None:
        self._candles_by_timeframe = {tf.upper(): cs for tf, cs in candles_by_timeframe.items()}
        self._futures: dict[str, object] = {}
        self.requested_timeframes: list[str] = []

    def register_response_future(self, key: str, future) -> None:
        self._futures[key] = future

    def cancel_response_future(self, key: str) -> None:
        self._futures.pop(key, None)

    async def publish(self, msg, triggered_by=None) -> None:
        if msg.event_type != EventType.CANDLES_REQUEST:
            return
        future = self._futures.get(str(msg.id))
        if future is None or future.done():
            return
        timeframe = str(msg.payload.get("timeframe", "M5")).upper()
        ...
        future.set_result({"candles": [_candle_to_response_dict(c) for c in candles], "error": None})


def _bus(m5: list[Candle] | None = None, h1: list[Candle] | None = None) -> _FakeCandleBus:
    return _FakeCandleBus({"M5": m5 or [], "H1": h1 or []})
...
async def test_short_tf_long_tf_placeholders_are_resolved() -> None:
    ...
    start = datetime(2026, 5, 7, 10, 0, tzinfo=UTC)
    m15 = [_make_candle(start + timedelta(minutes=15 * i), 1.17, 1.171, 1.169, 1.1705, "M15") for i in range(30)]
    h4 = [_make_candle(start + timedelta(hours=4 * i), 1.16, 1.161, 1.159, 1.1605, "H4") for i in range(60)]
    bus = _FakeCandleBus({"M15": m15, "H4": h4})
    ...
    # The bus received CANDLES_REQUESTs for the resolved timeframe strings, never the placeholders.
    assert "M15" in bus.requested_timeframes
    assert "H4" in bus.requested_timeframes
    assert "SHORT_TF" not in bus.requested_timeframes
    assert "LONG_TF" not in bus.requested_timeframes
